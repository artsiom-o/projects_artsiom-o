...
int main() {
    ...
    // Prompt user for the speed of the spaceship as a fraction of the speed of light (speed >= 0)
    printf("Please enter the speed of your spaceship as a fraction of the speed of light (speed >= 0): ");
    scanf("%lf", &speed);
    
    // Check if speed is a non-negative number
    if (speed < 0) {
        printf("Invalid input. The speed must be a non-negative number.\n");
        return 1;  // Exit program with error code 1
    }
    ...
    printf("\nFrom an observer's perspective on %s, it will take approximately %.2f years for %s to reach %s.\n", departure_point, time_earth, spaceship_name, starName);
     ...
     printf("Meanwhile, their friends and family back on %s will age by approximately %.2f years.\n", departure_point, time_earth);
