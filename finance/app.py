...
def buy():
    ...
    if request.method == 'POST':
        ...
        if not symbol:
            ...
            return apology('Must provide positive intiger number of shares')
...
def register():
    ...
    if request.method == "POST":
        if not request.form.get('username'):
            return apology('Must be providing USERNAME', 400)
        elif not request.form.get('password'):
            return apology('Must be providing PASSWORD', 400)
        elif not request.form.get('confirmation'):
            return apology('Must be providing CONFIRMATION', 400)

        elif request.form.get('password') != request.form.get('confirmation'):
            return apology('Sorry, PASSWORD do not match')


        rows = db.execute('SELECT * FROM users WHERE username = ?', request.form.get('username'))

        if len(rows) != 0:
            return apology('Sorry, this username already exists', 400)
...
def sell():
    ...
    if request.method == 'POST':
        ...
        shares = request.form.get('shares')
        if not symbol:
            return apology('Please, enter some symbols')
        elif not shares or not shares.isdigit() or int(shares) <= 0:
            return apology('Must provide positive numbers of shares')
        else:
            shares = int(shares)
...
def password():
    ...
    if request.method == "POST":
        ...
        hashPwd = generate_password_hash(new_password, method='pbkdf2:sha256', salt_length=8)

        db.execute("UPDATE users SET hash = ? WHERE id = ?", hashPwd, current_user_id)

        flash(f'You cahnges your password succesfully!')
        return redirect("/")
